...
// 安全bessel方法
double ModelSolver03::safe_bessel_k(
    int v, double x)
{
    if (x < 1e-15) x = 1e-15;
    try {
        ...
        const QMap<QString, double>& p)
{
    ...
    double pf_b = PWD_composite(
        z, fs1, fs2, M12, LfD, rmD,
        reD, nF, m_type);

    int sT = (int)m_type % 4;
    if (sT == 1) return pf_b;

    double CD = p.contains("cD") ?
                    p.value("cD") :
                    0.159 * p.value("C", 100.0) /
                        (p.value("phi", 0.05) * p.value("h", 10.0) * p.value("Ct", 5e-3) * L * L);
        ...
        ModelType type)
{
    ...
    if (n_fracs == 1) xwD[0] = 1.0;
    else {
        for (int k = 0; k < n_fracs; ++k)
            xwD[k] = 0.1 + 1.8 * (double)k /
                               (double)(n_fracs - 1);
    }
